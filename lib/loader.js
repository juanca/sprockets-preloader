...
function ensureRequire(context, requireFile) {
  ...
  if (assetFile) return importsLoader + '?module=>undefined,exports=>undefined,define=>undefined,this=>window!./' + path.relative(context, assetFile);

  var vendorFilePath = path.join(vendorPath, requireFile);
  var vendorFile = resolveFilePath(vendorFilePath);
  if (vendorFile) return importsLoader + '?module=>undefined,exports=>undefined,define=>undefined,require=>undefined,this=>window!./' + path.relative(context, vendorFile);
  ...
  if (gemFile) return importsLoader + '?module=>undefined,exports=>undefined,define=>undefined,this=>window!./' + path.relative(context, gemFile);
