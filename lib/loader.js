...
module.exports = function(source) {
  ...
  var lineParseReducer = function(outputLines, line) {
    ...
    if (requireDirectoryMatches) return outputLines.concat(transformers.requireTree(context, requireDirectoryMatches[3]));

    var requireSelfMatches = parsers.requireSelf(line);
    if (requireSelfMatches) return outputLines.concat(transformers.requireTree(source));
