...
function ensureRequire(context, requireFile) {
  ...
  // These should set the context to window but it is also used for gem files
  // which means that an absolute path to the imports-loader is needed :(
  ...
  if (vendorFile) return 'imports?module=>undefined,exports=>undefined,define=>undefined,require=>undefined,this=>window!./' + path.relative(context, vendorFile);

  var gemPath1 = require('child_process').spawnSync('bundle', ['show', requireFile]).output[1].toString().trim();
  var gemPath2 = require('child_process').spawnSync('bundle', ['show', requireFile + '-js']).output[1].toString().trim();
  gemPaths = [
  ...
  // These should also turn off imports of module, exports, define, require, and set the context to window
  // However, it looks like an absolute path to the imports-loader is needed :(
