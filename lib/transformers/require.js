...
function getGemAssetPath(requireFile) {
  ...
  var gemPath1 = childProcess.spawnSync('bundle', ['exec', 'bundle', 'show', gem]).output[1].toString().trim();
  // Edge case for some gems (brainstem -> brainstem-js)
  var gemPath2 = childProcess.spawnSync('bundle', ['exec', 'bundle', 'show', gem + '-js']).output[1].toString().trim();
