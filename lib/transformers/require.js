...
function ensureRequire(context, requireFile) {
  ...
  if (assetFile) return importsDecorator(path.relative(context, assetFile), { require: true });

  var vendorFilePath = path.join(vendorPath, requireFile);
  var vendorFile = resolveFilePath(vendorFilePath);
  if (vendorFile) return importsDecorator(path.relative(context, vendorFile));
  ...
  if (gemFile) return importsDecorator(path.relative(context, gemFile), { require: true });
...
module.exports = function requireTransformer(context, requireFile) {
  ...
  if(newRequireFilePath) {
    return requireDecorator(newRequireFilePath);
  } else {
